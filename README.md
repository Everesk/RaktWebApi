...
- `GET /events/top` использует ключ `events:top10` и содержит рейтинг по доле проданных мест: `(total_seats - available_seats) / total_seats`. Его кеш не инвалидируется при изменении отдельных событий или бронированиях: небольшое устаревание рейтинга допустимо, а дополнительные операции с Redis на каждое бронирование не нужны.
