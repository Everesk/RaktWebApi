...
Параметры пути:
- eventId - идентификатор существующего события
...
Это означает, что после `POST /bookings/{eventId}` статус брони может измениться спустя несколько секунд, и `GET /bookings/{id}` покажет уже обновлённое состояние.
