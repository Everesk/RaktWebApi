...
- `GET /events/top` — десять событий с наибольшей долей проданных мест;
...
Сервис Events использует Redis через интерфейс `ICache` из слоя Application. Реализация Redis находится в Infrastructure, поэтому провайдер кеша можно заменить без изменения прикладных сценариев. Если Redis недоступен, ошибка записывается в лог, а запрос продолжает работу с PostgreSQL.

- `GET /events/{id}` использует ключ `event:{id}`. Выбрана стратегия обновления при записи: после успешного создания или обновления события его актуальные данные записываются в кеш; после удаления ключ удаляется. Обработчики Kafka также обновляют этот ключ только после сохранения резервирования или возврата места в БД.
- `GET /events/top` использует ключ `events:top10` и содержит рейтинг по доле проданных мест. Его кеш не инвалидируется при изменении отдельных событий или бронированиях: небольшое устаревание рейтинга допустимо, а дополнительные операции с Redis на каждое бронирование не нужны.
- Время жизни обеих записей задаётся параметром `Cache:TimeToLiveMinutes` в `Rakt.EventsService.Presentation/appsettings.json` и составляет 5 минут. Это ограничивает период возможного устаревания при недоступности обновления кеша, но заметно снижает количество обращений к БД для популярных запросов.
